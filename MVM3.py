...
class reg(object):
    ...
    def model(size_input , size_output):  
      ...
      if(i==0):
        def my_leaky_relu(x):
          return tf.keras.activations.elu(x, alpha= 0.01)
        layer1 = 2*size_input
        optimizer = tf.keras.optimizers.Adam(learning_rate= 0.01)
        #optimizer = tf.keras.optimizers.SGD()
        model1 = Sequential()
        model1.add(Dense( layer1 ,activation = my_leaky_relu , input_shape=(size_input,)))
    ...
    def reg_train(t1 , t2 , thres):
        ...
        if( i == 0):
          ...
          tem = order>1
          tem2 = order<1
          order = order*tem2 + (order*tem)*(0.1)
          order=order.reshape(1,order.size)
          order = np.reshape(order , (1,order.size))*(100)
          np.save('ls2',ls1)
          np.save('ls4',ls2)
        else:
          ls2=np.load('ls2.npy')
        ...
        start = time.time()
        history = model.fit(order,data, epochs=300)
        end = time.time()
        print(end - start)
